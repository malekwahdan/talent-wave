    ...
    <div class="row">
      ...
      <div class="d-flex align-items-center justify-content-between mb-4">
                        <div id="authorsSearchContainer">
                                            <form method="GET" action="{{route('document.index')}}" id="authorsDepartmentForm" class="authorsForm">
                                                <select id="authorsDepartmentFilter" name="filter" onchange="this.form.submit()">
                    ...
                    <td class="align-middle text-center">
                        ...
                        <form method="POST" action="{{ route('document.approve',['id'=> $document->id]) }}" class="d-inline">
                            ...
                            <form method="POST" action="{{ route('document.rejecte',['id' => $document->id]) }}" class="d-inline">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="btn btn-sm btn-outline-danger">Rejecte</button>
                            </form>
                            <a href="{{ route('documents.view', $document->id) }}" class="btn btn-sm btn-outline-info">Show</a>
                        </div>
                    </td>
                  </tr>
                  @endforeach
